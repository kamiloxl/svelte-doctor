...
describe("applySuppressions", () => {
  it("suppresses next line via // comment", () => {
    ...
    const diagnostics = [makeDiag(file, 3, "svelte-doctor/no-fetch-in-effect")];
  ...
  it("suppresses next line via HTML comment", () => {
    ...
    const diagnostics = [makeDiag(file, 2, "svelte-doctor/no-unsafe-html-binding")];
  ...
  it("supports comma-separated rule list", () => {
    const file = "/x/a.svelte";
    const source = [
      "// svelte-doctor-disable-next-line svelte-doctor/a, svelte-doctor/b",
      "broken();",
    ].join("\n");
    const filtered = applySuppressions(
      [makeDiag(file, 2, "svelte-doctor/a"), makeDiag(file, 2, "svelte-doctor/b")],
      new Map([[file, source]]),
    );
    expect(filtered).toHaveLength(0);
  });
  ...
  it("does not suppress unrelated rules", () => {
    ...
    const filtered = applySuppressions(
      [makeDiag(file, 2, "svelte-doctor/prefer-derived-over-effect")],
      new Map([[file, source]]),
    );
  ...
  it("accepts bare rule id without prefix", () => {
    const file = "/x/a.svelte";
    const source = "// svelte-doctor-disable-next-line no-fetch-in-effect\nfetch();";
    const filtered = applySuppressions(
      [makeDiag(file, 2, "svelte-doctor/no-fetch-in-effect")],
      new Map([[file, source]]),
    );
    expect(filtered).toHaveLength(0);
  });
...
describe("applySuppressionsAndDetectUnused — near-miss", () => {
  it("attaches a near-miss hint when comment is too far above the diagnostic", () => {
    const file = "/x/a.svelte";
    const source = [
      "// svelte-doctor-disable-next-line svelte-doctor/no-fetch-in-effect", // 1
      "let unrelated = 1;",                                                   // 2
      "let other = 2;",                                                       // 3
      "$effect(() => fetch('/x'));",                                          // 4
    ].join("\n");
    const diagnostics = [makeDiag(file, 4, "svelte-doctor/no-fetch-in-effect")];
  ...
  it("attaches a hint when adjacent comment lists wrong rule id", () => {
    const file = "/x/a.svelte";
    const source = [
      "// svelte-doctor-disable-next-line svelte-doctor/no-other-rule",
      "$effect(() => fetch('/x'));",
    ].join("\n");
    const result = applySuppressionsAndDetectUnused(
      [makeDiag(file, 2, "svelte-doctor/no-fetch-in-effect")],
...
describe("applySuppressionsAndDetectUnused — unused-disable", () => {
  it("reports unused suppression directives when nothing fired at the next line", () => {
    ...
    expect(result.unusedDisables[0].ruleId).toBe(
      "svelte-doctor/unused-disable-directive",
    );
...
describe("applySuppressionsAndDetectUnused — stacks", () => {
  it("respects stacked disable comments (each rule on its own line)", () => {
    ...
    const source = [
      "// svelte-doctor-disable-next-line svelte-doctor/no-fetch-in-effect", // 1
      "// svelte-doctor-disable-next-line svelte-doctor/no-cascading-state-in-effect", // 2
      "$effect(() => { fetch('/x'); a = 1; b = 2; });",                                // 3
    ].join("\n");
    const result = applySuppressionsAndDetectUnused(
      [
        makeDiag(file, 3, "svelte-doctor/no-fetch-in-effect"),
        makeDiag(file, 3, "svelte-doctor/no-cascading-state-in-effect"),
      ],
      new Map([[file, source]]),
    );
    expect(result.diagnostics).toHaveLength(0);
  ...
  it("issues one unused-disable per stack, not per comment", () => {
    ...
    const source = [
      "// svelte-doctor-disable-next-line svelte-doctor/a, svelte-doctor/b",
      "// svelte-doctor-disable-next-line svelte-doctor/c",
      "let nothing = 1;",
    ].join("\n");
    const result = applySuppressionsAndDetectUnused(
      [],
...
describe("applySuppressionsAndDetectUnused — multiline tag opener", () => {
  it("suppresses diagnostics inside multiline opening tag", () => {
    const file = "/x/a.svelte";
    const source = [
      '<!-- svelte-doctor-disable-next-line svelte-doctor/no-href-javascript -->',
      "<a",
      '  href="javascript:void(0)"',
      ">x</a>",
    ].join("\n");
    const result = applySuppressionsAndDetectUnused(
      [makeDiag(file, 3, "svelte-doctor/no-href-javascript")],
      new Map([[file, source]]),
    );
    expect(result.diagnostics).toHaveLength(0);
    expect(result.unusedDisables).toHaveLength(0);
